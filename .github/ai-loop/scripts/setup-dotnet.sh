#!/usr/bin/env bash
# setup-dotnet.sh
# 安裝腳本 #40 | 將 ai-loop 引擎複製到 .NET 專案
# Usage: ./scripts/setup-dotnet.sh [TARGET_PROJECT_PATH]
...
if [ -z "$TARGET_PATH" ]; then
    ...
    echo "Example: $0 /path/to/imxframework"
    exit 1
fi
...
# 驗證是 .NET 專案
if ! ls "$TARGET_PATH"/*.sln "$TARGET_PATH"/*.csproj 2>/dev/null | grep -q .; then
    echo "❌ Not a .NET project (no *.sln or *.csproj found): $TARGET_PATH"
    exit 1
fi
...
echo "=== Installing ai-loop engine to .NET project ==="
...
echo ""
echo "--- .NET Adapter ---"
DOTNET_SRC="$ADAPTER_SRC/dotnet"
for f in detector.md commands.yaml error-parser.md review-dimensions.md; do
    install_file "$DOTNET_SRC/$f" ".github/ai-loop/adapters/dotnet" "$f"
done
...
echo ""
echo "============================================="
echo "✅ ai-loop engine installed to .NET project"
echo "   $TARGET_PATH"
echo ""
echo "Next steps:"
