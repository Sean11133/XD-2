...
- **開發環境**：Python 3.10-3.14，內部 Nexus 倉庫
...
> **⚠️ 新專案強制規則**：建立 config.yaml 前，**必須先向使用者詢問以下四項識別資訊**，
> 取得回答後才能填入；禁止使用 `MY_APP`、`MY_SYSTEM` 等佔位符範本名稱建立真實專案：
>
> | 欄位       | 說明                                             | 範例                         |
> | ---------- | ------------------------------------------------ | ---------------------------- |
> | `system`   | 對應 `IMX_SYSTEM`，系統代碼                      | `IRS`、`AVM`、`IANALYSIS`    |
> | `app`      | 對應 `IMX_APP`，應用程式代碼                     | `MODEL_MONITOR`、`DATA_SYNC` |
> | `app_type` | 應用類型，**僅允許** Schedule \| Listener \| Web | `Schedule`                   |
> | `section`  | 部門代碼，**必填**                               | `MK22`、`MK20`               |
